...
import { EApplicationStatus,ITableData, tableStaticDataApplications } from 'components/Table/table.interface';
...
const Applications: React.FC = () => {
    const [tableData, changeTableData] = React.useState<ITableData>(tableStaticDataApplications);
    ...
    return (
        <>
        {
            !isNewApplication? <Table onSearchAddClick={()=>onSarchAdd(true)} tableCheckedData={tableCheckData} tableData={tableData}>
            <TableBody>
                {tableData?.tbodyData &&
                    tableData?.tbodyData.map((data) => (
                        <TableTr isChecked={data.checked} key={data.id}>
                            ...
                            <TableTd>
                                <div className={data.status==='Qəbul edildi'?'text status-active':'text'}>{data.status}</div>
                            </TableTd>
                            <TableTd>
